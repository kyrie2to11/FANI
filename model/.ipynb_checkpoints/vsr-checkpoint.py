...
from tensorflow.keras.layers import Input, Conv2D, Multiply, Add, LayerNormalization, Lambda, LeakyReLU, BatchNormalization, MaxPooling2D, UpSampling2D
...
def PartialConv(inputs, in_channels, n_div=2):
    ...
    # feature maps split acording to calculated dim_conv/dim_untouched
    x1, x2 = tf.split(x, [dim_conv, dim_untouched], axis=-1)

    # large receptive filed partial depth-wise convolution
    x1 = Conv2D(dim_conv, 5, strides=1, padding='SAME', groups=dim_conv)(x1)
    x1 = Conv2D(dim_conv, 5, strides=1, padding='SAME', groups=dim_conv, dilation_rate=3)(x1)
...
def FlashModule(inputs, in_channels, n_div=2, mix_ratio=2):
...
def PPA(inputs, in_channels, n_div=2):
    ...
    attn = Conv2D(dim_conv, 3, strides=1, padding='SAME', groups=dim_conv, activation='sigmoid')(attn)
    x1 = Multiply()([attn, x1])
...
def MSDB(inputs, in_channels=16):
    # information multi-stage distillation block
    out_1 = FlashModule(inputs, in_channels)
    out_2 = FlashModule(out_1, in_channels)
    out_3 = FlashModule(out_2, in_channels)
    ...
    out = Conv2D(in_channels, 1, strides=1, padding='SAME')(out) # channel fuse: 16 -> 16
...
def AttentionAlign(x, ref, dim=3):
    ...
    project_out = Conv2D(dim, 1, strides=1, padding='SAME')

    # inputs LayerNormalization -> incur tflite bug
...
def FunctionalMVSR(inputs, num_out_ch=3, scale=4, num_feat=16, num_blocks=3):
    x_pre = inputs[:, :, :, :3]
    x = inputs[:, :, :, 3:6]
    x_nex = inputs[:, :, :, 6:]
    shape = tf.shape(x)
    ...
    x_forward = AttentionAlign(x, ref)

    # shallow feature extraction
    x_forward = BSConv(x_forward, num_feat)
    
    # feature alignment 
    # x_forward = FeatureAlign(x_forward)

    # deeper feature extraction
    for i in range(num_blocks):
        x_forward = MSDB(x_forward, in_channels=num_feat)
...
def MVSR():
    ...
    return Model(inputs=x, outputs=FunctionalMVSR(x))